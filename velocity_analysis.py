...
def mouse_click(event):
    ...
    my_label.place(x=event.x-6, y=event.y-6)
...
#   Leer segy con obspy
    ...
    ffid = (st.traces[0].stats.segy.trace_header.original_field_record_number)
    cmp = cmp_entry.get()
    fold = fold_entry.get()
    ensemble = (st.traces[47].stats.segy.trace_header.trace_number_within_the_ensemble)    
    SG = no_cmp.get() // 2
    min_vel = min_vel_entry.get()
    max_vel = max_vel_entry.get()
